...
import {
  ...
  LotMediaRow, mediaUrl, recordCustody, resolveLotHold, uploadMedia
...
export function LotDetailPage(): JSX.Element {
  ...
  const upload = async (e: ChangeEvent<HTMLInputElement>): Promise<void> => {
    ...
    try {
      ...
      await addLotMedia(id, { mediaObjectId: stored.id, purpose: 'LOT_PHOTO' });
      setNotice('Photo attached to lot.');
      await load();
    } catch (err) {
  ...
  return (
    <main className="app-shell" data-testid="lot-detail-page">
      ...
      <section className="panel" data-testid="lot-balances">
        ...
        <p className="hint">
          QC: accepted {lot.qc_accepted_qty ?? 0} · rejected {lot.qc_rejected_qty ?? 0} · held {lot.qc_held_qty ?? 0}
      ...
      <section className="panel" data-testid="lot-media-panel">
        ...
        <div className="inline-form">
          <input type="file" accept="image/*" capture="environment" data-testid="lot-media-input"
            disabled={busy} onChange={(e) => void upload(e)} />
        </div>
