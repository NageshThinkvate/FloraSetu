...
- **Logistics = monitor + support, never execution** (ADR-012 binding): the ops logistics workspace is read/exception-visibility only — no driver/vehicle assignment, no milestone marking, no POD submission. Phase 5 denials remain and are regression-tested (phase5 L-suite + phase6 S5a–S5c: PROCUREMENT_OPS / FINANCE_OPS / SUPPORT_AGENT all rejected on assign-driver, unassign-driver, `DRIVER_ASSIGNMENT_PARTNER_ONLY` on the legacy assign endpoint, and 403/404 on every partner milestone + partner POD path).
...
- **Derived exceptions vs persistent cases.** SLA breaches, coverage gaps and stalled states are DERIVED from authoritative state and can never be falsified by "resolving" them; persistent case lifecycles reuse existing models (claims.claims, logistics.shipment_exceptions). Resolving an exception record never mutates source business state.
- **Support triage is read-only in Phase 6** (owner ruling): SUPPORT_AGENT holds `claim.read` — claims/evidence/timeline visibility across organizations with NO adjudication, liability, refund/credit or financial-resolution authority; the backend exposes no safe non-adjudicative triage mutations, so none were invented for the UI. Claim decision controls render only for `claim.manage` holders (phase6 S3a–S3c).
- **Finance dual control unchanged** (owner ruling): recorder ≠ verifier (§29) enforced server-side; payout-freeze state surfaces read-only in the finance workspace from the existing audited bank-change workflow (ADR-004) — no unilateral freeze/unfreeze capability was added (phase6 S4a–S4c).
- **Phase 6 workspaces (UI):** `/ops/exceptions` (board + scoped search + audited CSV export), `/ops/procurement` (existing desk), `/ops/orders` (monitor), `/ops/logistics` (monitor, read-only), `/ops/claims` + `/ops/claims/:id` (evidence-first; decision panel permission-gated), `/ops/finance` (verification queues + record forms + freeze visibility). Role-filtered nav; legacy `/ops/sourcing`/`/ops/quality` redirected, Quality removed from the shell.
- Implementation: migration 016 (additive, reversible) + `test/e2e/phase6-ops-control-tower.e2e-spec.ts` (S1–S6, 13 tests). Final gate: 25 suites / 269 tests green; web typecheck + lint + build green; testing agent iteration_19 (9/10 flows PASS; support flow blocked by a seed gap — `seed-phase2-personas.ts` assigned the SUPPORT_AGENT role without the org membership — fixed in the seed and live DB, then re-verified: support nav isolation, claims 200, transition 403).
...
## ADR-014 — Admin Control Plane & Governance Boundary — RESOLVED — OWNER APPROVED (Phase 7 owner directive)
- **Admin governs the platform; Admin is not a marketplace participant.** The Admin Control Plane (`/admin/*`) governs organizations, KYB verification, users & access, seeded roles (read-only), approved runtime configuration, feature flags, security state and audit inspection. It contains no procurement/supplier/logistics/finance/claims execution — PLATFORM_ADMIN holds no `logistics.assign_driver`, and partner milestone/POD endpoints stay partner-only (regression: phase7 S5). Claim financial resolution stays state-machine-bound (S6).
- **KYB review is controlled and audited; the state machine is unchanged** (NOT_STARTED → IN_REVIEW → VERIFIED/REJECTED, resubmission from REJECTED). "Request Correction" is NOT a new state: it is `REJECTED` + structured `reason_code = CORRECTION_REQUIRED` + mandatory correction note (migration 017 adds `identity.verification_history.reason_code`); genuine rejections require a structured reason code server-side (400 otherwise). UX renders REJECTED+CORRECTION_REQUIRED as "Correction requested". Every decision writes immutable verification history + audit + security audit; prior cycles are never overwritten.
- **KYB documents are protected evidence**: reviewer detail (`GET /admin/kyb/:orgId/detail`, `kyb.review` only) returns short-lived HMAC-signed URLs (authorize-before-sign; raw endpoint rejects unsigned access); document access is recorded in the security audit stream. No public URLs, no general-search exposure (S10: staff search never returns bank/sensitive types).
- **Sensitive financial/account changes retain dual control** (ADR-004 unchanged): payout-freezing bank changes surface read-only in the control plane; a single actor can never request AND approve the same protected change (S9: second approval by the same actor → 409). `admin.user.manage` (migration 017) is deliberately narrow — suspend/reactivate with mandatory reason, self-suspension blocked, login + org-context blocked for suspended users (OrgContextService rejects non-ACTIVE users); no IAM mutation, no MFA changes, no credential access.
- **Roles stay seeded/static** (Build 1 unchanged): the Roles & Permissions page is a read-only matrix; no create/edit/delete role APIs exist (404 proven, A15); org-level role assignment keeps the Build-1 escalation allowlist (platform roles never org-assignable, A12–A14).
- **Configuration & flags are audited, never silent**: only pre-existing keys are writable (404 on unknown keys); config changes record old/new value + actor + reason in immutable audit (A16/A17); flag changes are effective-dated (close window, open new — never overwrite) and never bypass RBAC/KYB/domain invariants (A19: flag ON + missing permission → still 403).
- **Audit logs are immutable and inspectable**: search by action/object/actor/trace/date (`audit.read`), authorized CSV export (itself audited, `admin.audit_export`; no secrets/bank data in the stream), no edit/delete path exists and the DB trigger rejects tampering (A21).
- **ADR-011 intact**: no Partner QC reintroduction — Admin governs master-data definitions only; configuration is not execution. ADR-010/012/013 unchanged.
- Implementation: migration 017 (additive, reversible; permissions `admin.user.read`/`admin.user.manage`/`flag.manage` → PLATFORM_ADMIN; `verification_history.reason_code`) + `test/e2e/phase7-admin-control-plane.e2e-spec.ts` (26 tests: A1–A22 + S1–S11). Members list reads extended to platform-privileged roles via existing `assertOrgAccessOrPlatform`. Final gate: 26 suites / 300 tests green; web typecheck + lint + build green; testing agent iteration_20 (14/15 flows PASS; 5 UX issues fixed by main agent — API client 204/empty-body guard, users search submit, self-suspend UI guard, per-org audit filter by object_id — and self-verified).
