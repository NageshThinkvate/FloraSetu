...
- P0 (PRE-EXISTING Build 4 regression, owner decision REQUIRED before Phase 2): `no-cross-table` FIXED correctly 2026-09 (regex tightened to exclude contract property access like `this.supply.x()` — 32 of 33 hits were false positives; 1 real violation in custody.service.ts rerouted via SupplyInventory contract; suite PASS, e2e 196/196 green). `no-cycles` REMAINS FAILING: two contracts-level 2-cycles — ① supply-inventory→order-allocation (`buyerHasLotAllocation` tenant-visibility read in lots.service) ↔ order-allocation→supply-inventory (reserve/release), ② order-allocation→logistics-coldchain (`hasBlockingException`, ADR-002/§21 acceptance hold) ↔ logistics-coldchain→order-allocation (pack/dispatch). Both reverse edges are legitimate Master-spec controls; breaking them needs either an ADR-documented exception pair (docs/04 rule 5 escape hatch) or outbox event projections (write-path changes). STOPPED for owner decision — Phase 2 blocked per authorization.
