...
- P0 (PRE-EXISTING, unrelated to Phase 1): Build 0 architecture suites `no-cycles` + `no-cross-table` FAIL on Build 4 modules (tower.service foreign-schema refs; analytics-controltower↔order-allocation↔supply-inventory/logistics cycle). Zero Phase 1 backend changes — regression introduced during Build 4 when gate ran e2e-only. Needs owner decision: refactor modules vs amend scan rules.
